...
export default function AdminPage() {
  const [aba, setAba] = useState<'imovel' | 'leads'>('imovel')
  ...
  const verificacaoSessaoIniciada = useRef(false)
  ...
  useEffect(() => {
    if (verificacaoSessaoIniciada.current) return
    verificacaoSessaoIniciada.current = true
  ...
  const carregarLeads = useCallback((pagina: number) => {
    setLeadsCarregando(true)
    fetch(`/api/admin/leads?pagina=${pagina}`)
      .then(res => res.json())
      .then(data => {
        setLeads(data.leads ?? [])
        setLeadsTotal(data.total ?? 0)
        setLeadsTotalPaginas(data.totalPaginas ?? 1)
        setLeadsPagina(pagina)
        setLeadsCarregando(false)
      })
      .catch(() => setLeadsCarregando(false))
  }, [])

  useEffect(() => {
    if (!desbloqueado || aba !== 'leads') return

    let ativo = true
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLeadsCarregando(true)

    fetch(`/api/admin/leads?pagina=${leadsPagina}`)
      .then(res => res.json())
      .then(data => {
        if (!ativo) return
        setLeads(data.leads ?? [])
      ...
      .finally(() => {
        if (ativo) setLeadsCarregando(false)
      })

    return () => {
      ativo = false
    }
  }, [desbloqueado, aba, leadsPagina])

  async function desbloquear(e: FormEvent<HTMLFormElement>) {
    e.preventDefault()
    const token = adminToken.trim()
    if (!token) { setStatus('error'); setMsg('Informe o token de admin.'); return }
    setStatus('loading')
    try {
  ...
  async function consultarCep() {
    if (!form.cep?.trim()) { setStatus('error'); setMsg('Informe um CEP valido.'); return }
  ...
  async function salvar() {
    ...
    if (faltando) { setStatus('error'); setMsg(`Preencha o campo obrigatorio: ${faltando.label}.`); return }
    if (arquivos.length < 1) { setStatus('error'); setMsg('Adicione ao menos uma foto do imovel.'); return }
    setStatus('loading')
    let imagensEnviadas: UploadedImage[] = []
    try {
      const orderedFiles = [arquivos[principalIndex], ...arquivos.filter((_, index) => index !== principalIndex)]
      const imageData = new FormData()
      orderedFiles.forEach(file => imageData.append('files', file))
      const uploadRes = await fetch('/api/admin/property-images', { method: 'POST', body: imageData })
  ...
  if (!desbloqueado) {
    return (
      <div style={{ maxWidth: '440px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        <h1 style={{ fontFamily: 'var(--font-dm-serif)', fontSize: '1.75rem', marginBottom: '0.5rem' }}>Painel Admin</h1>
        <form onSubmit={desbloquear} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.5rem', padding: '1.25rem' }}>
          <div>
            <label htmlFor="admin-token" style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' }}>Token de admin</label>
            <input id="admin-token" type="password" value={adminToken} onChange={(e) => setAdminToken(e.target.value)} autoComplete="current-password" />
          </div>
    ...
    <div style={{ maxWidth: '980px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '0' }}>
        <button
          ...
          style={{ borderBottom: aba === 'imovel' ? '2px solid var(--primary)' : '2px solid transparent', borderRadius: 0, paddingBottom: '0.75rem' }}
        >
          Novo Imovel
        </button>
        <button
          ...
          style={{ borderBottom: aba === 'leads' ? '2px solid var(--primary)' : '2px solid transparent', borderRadius: 0, paddingBottom: '0.75rem' }}
        >
          <Users size={15} aria-hidden="true" /> Leads {leadsTotal > 0 && `(${leadsTotal})`}
        </button>
      </div>
      ...
      {aba === 'leads' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{leadsTotal} contato{leadsTotal !== 1 ? 's' : ''} recebido{leadsTotal !== 1 ? 's' : ''}</p>
            <button className="btn btn-ghost" onClick={() => carregarLeads(leadsPagina)} disabled={leadsCarregando}>
              {leadsCarregando ? 'Carregando...' : 'Atualizar'}
            </button>
          </div>

          {leadsCarregando && <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Carregando leads...</p>}
          ...
          {!leadsCarregando && leads.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {leads.map(lead => (
                <div key={lead.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.25rem', display: 'grid', gridTemplateColumns: '1fr auto', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{lead.nome}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{lead.telefone}{lead.email ? ` - ${lead.email}` : ''}</div>
                    {lead.imoveis && (
                      <div style={{ fontSize: '0.8rem', color: 'var(--primary)' }}>
                        {lead.imoveis.titulo}
                      </div>
                    )}
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>{lead.mensagem}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>{formatarData(lead.created_at)}</div>
                  </div>
                  <div>
                    <span style={{ background: STATUS_COLORS[lead.status] ?? 'var(--bg-elevated)', borderRadius: 'var(--radius-sm)', fontSize: '0.75rem', fontWeight: 600, padding: '0.25rem 0.6rem' }}>
          ...
          {leadsTotalPaginas > 1 && (
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginTop: '1.5rem' }}>
              <button className="btn btn-ghost" onClick={() => setLeadsPagina(page => Math.max(page - 1, 1))} disabled={leadsPagina <= 1}>Anterior</button>
              <span style={{ padding: '0.5rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{leadsPagina} / {leadsTotalPaginas}</span>
              <button className="btn btn-ghost" onClick={() => setLeadsPagina(page => Math.min(page + 1, leadsTotalPaginas))} disabled={leadsPagina >= leadsTotalPaginas}>Proxima</button>
            </div>
          )}
        </div>
      )}
      ...
      {aba === 'imovel' && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
          <div style={{ gridColumn: '1 / -1' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Complete dados, fotos e localizacao antes de publicar.</p>
          </div>
          ...
          {CAMPOS.map((campo) => (
            <div key={campo.key} style={{ gridColumn: campo.span === 2 ? '1 / -1' : undefined }}>
              ...
              {campo.type === 'select' ? (
                ...
                <textarea id={`admin-${campo.key}`} rows={4} value={form[campo.key] ?? ''} onChange={(e) => setForm({ ...form, [campo.key]: e.target.value })} style={{ resize: 'vertical' }} />
              ) : (
                <input id={`admin-${campo.key}`} type={campo.type} step={campo.key === 'latitude' || campo.key === 'longitude' ? 'any' : undefined} value={form[campo.key] ?? ''} onChange={(e) => setForm({ ...form, [campo.key]: e.target.value })} />
              )}
              {campo.key === 'cep' && (
          ...
          <div style={{ gridColumn: '1 / -1' }}>
            ...
            <input id="admin-fotos" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => selecionarFotos(event.target.files)} style={{ position: 'absolute', width: '1px', height: '1px', overflow: 'hidden', opacity: 0 }} />
          </div>
            ...
            <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.75rem' }}>
              {previews.map((preview, index) => (
                <div key={`${preview.arquivo.name}-${index}`} style={{ border: index === principalIndex ? '2px solid var(--primary)' : '1px solid var(--border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
                  <div role="img" aria-label={`Preview ${index + 1}: ${preview.arquivo.name}`} style={{ aspectRatio: '4 / 3', backgroundImage: `url("${preview.url}")`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
          ...
          {status !== 'idle' && (
            <div style={{ gridColumn: '1 / -1', padding: '0.875rem', borderRadius: 'var(--radius-sm)', background: status === 'success' ? 'rgba(34,197,94,0.1)' : status === 'error' ? 'rgba(239,68,68,0.1)' : 'var(--bg-elevated)', color: status === 'success' ? 'var(--success)' : status === 'error' ? 'var(--danger)' : 'var(--text-muted)', fontSize: '0.875rem' }}>
